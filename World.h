...
HitResult World::hit(Ray& ray, float min_t, float max_t)
{
    ...
    // Answer:
    for (shared_ptr<Sphere> s : m_spheres){
        if(s.get()->hit(ray,min_t,max_t).m_isHit==true){
            hit_result=s.get()->hit(ray,min_t,max_t);
            break;
        }
    }//get the first hit
    for (shared_ptr<Sphere> s : m_spheres){
        if(s.get()->hit(ray,min_t,max_t).m_isHit==true && s.get()->hit(ray,min_t,max_t).m_t<hit_result.m_t){
            hit_result=s.get()->hit(ray,min_t,max_t);
        }
    }//get the nearest hit 
    return hit_result;
